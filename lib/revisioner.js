...
var Revisioner = (function() {
    ...
    var Revisioner = function(options) {
        ...
        this.revCodeLength = parseInt(this.options.revCodeLength || 8);

        if (this.revCodeLength < 1 || this.revCodeLength > 13) {
            this.revCodeLength = 8;
        }

        this.versionKey = this.options.versionKey || '';

    };

    util.inherits(Revisioner, events.EventEmitter);
    ...
    Revisioner.prototype.buildManifestFile = function() {
        var list = {},
            contents,
            file;

        var reg = new RegExp('[.](' + this.options.mainfestType + ')$', 'gi');
        for (var key in this.manifest) {
            var value = this.manifest[key],
                revCode = this.files[key].revCode;
            if (key.match(reg)) {
                list[key] = revCode;
            }
        }
        if (isEmptyObject(list) || !this.cfgFile) {
            return;
        }

        file = this.cfgFile;
        contents = String(file.contents);
        //path = Path.join(this.pathBase, path);

        list['__isQuery__'] = '1';
        contents = contents.replace('("$$rev-manifest$$")', new Buffer(JSON.stringify(list, null, 2)));
    ...
    Revisioner.prototype.processFile = function(file) {
        ...
        if (!this.pathBase) {
            ...
            for (var level = 0, length = levelsFile.length; level < length; level++) {
                ...
                if (level < levelsBase.length && level < levelsFile.length && levelsBase[level] == levelsFile[level]) {
        ...
        file.revCode = String(file.stat.mtime.getTime()).substring(0, this.revCodeLength);
    ...
    Revisioner.prototype.run = function() {
        ...
        this.cfgPath = this.Tool.get_relative_path(this.pathBase, Path.resolve(this.pathBase, this.options.configFilePath));

        var file = this.files[this.cfgPath];
        if (file) {
            file.revHashOriginal = this.Tool.md5(String((new Date()).getTime()));
        }
        this.cfgFile = file;
    ...
    Revisioner.prototype.revisionFilename = function(file) {
        ...
        var pathOriginal = this.Tool.get_relative_path(file.base, file.path);
        var pathRevisioned = this.Tool.get_relative_path(file.base, file.path, true);
        this.manifest[pathOriginal] = hashValue;
    ...
    Revisioner.prototype.updateReferences = function(file) {
        ...
        for (var pathReference in file.revReferencePaths) {
            ...
            if (this.shouldUpdateReference(reference.file)) {
                contents = contents.replace(reference.regExp, function(pathReference) {
                    var versionCode = versionKey + reference.file.revCode + '=1';
                    var urlArray = pathReference.replace(/"|'/g, '').split('?');
                    var locaHashArray = (urlArray[1] || '').split('#');
                    var argArray = (locaHashArray[1] || locaHashArray[0]).split('&');
                    ...
                    var s = '';
                    if (/^'/.test(pathReference)) {
                        s = "'";
                    } else if (/^"/.test(pathReference)) {
                        s = '"';
                    }
                    return s + urlArray[0] + '?' + argArray.join('&') + s;
                });
            }
